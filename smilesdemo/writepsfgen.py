...
def clearC1(mol, residue, idxs):
    ...
    for idx in idxs:
        ...
        if atom.HasProp("name") and atom.HasProp("residue") and (atom.GetProp("name") == "C1") and \
                (int(atom.GetProp("residue")) == residue):
...
for molnum, mol in enumerate(testmol):
    ...
    for dimer in dimers:
        ...
        for match in matches:
            ...
            # For the rest we need to look at the match index. These will tell us what side is residue "1" and
            # which is residue "2" for patching.
                ...
                fout.write("patch B5%s %s:%s %s:%s\n" % (atomresid2.GetProp('restype')[0], segname,
                                                         atomresid1.GetProp('residue'), segname,
                                                         atomresid2.GetProp('residue')))
            elif dimer.GetProp('_Name') == '4O5':
                atomresida = mol.GetAtomWithIdx(match[5])
                atomresidb = mol.GetAtomWithIdx(match[7])
                if atomresida.HasProp("name") and atomresida.GetProp("name") == "C4" and atomresidb.HasProp("name") \
                        and atomresidb.GetProp("name") == "C4":
                    ...
                    exit()
                fout.write("patch 4O5 %s:%s %s:%s\n" % (segname, atomresid1.GetProp('residue'), segname,
                                                        atomresid2.GetProp('residue')))
                ...
                fout.write("patch %s %s:%s %s:%s\n" % (dimer.GetProp('_Name'), segname, atomresid1.GetProp('residue'),
                                                       segname, atomresid2.GetProp('residue')))
                ...
                fout.write("patch SPIR %s:%s %s:%s\n" % (segname, atomresid1.GetProp('residue'),
                                                         segname, atomresid3.GetProp('residue')))
