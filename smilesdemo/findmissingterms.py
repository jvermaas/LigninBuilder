...
def findmissingparameters(self, parmset):
        """
        Based on ParmEd's "load_parameters" routine, this goes through the loaded psf and determines what parameters are missing.
...
def missingparameterstext(psflist, parameterlist):
	#Pass a directory that has psfs in it and a list of parameter files that contain analogous parameters, and return a string containing all the parameters that need to be added for simulation.
	params = CharmmParameterSet()
	for p in parameterlist:
		params.read_parameter_file(p)
	returntext = "! This file was written by analogy from the following input parameter files: %s\n\n" % str(prmlist)[1:-1]
	...
	if len(missingset[0]):
		print("The following atomtypes are missing nonbonded terms from the input parameter files. This is probably an input problem, and so we are exiting.")
		print(missingset[0])
		exit()
	for i in range(1,4):
	...
	for missingbondtype in missingset[1]:
		...
		for k in params.bond_types:
			...
			if ed < mindistance:
				replacementtype = k
				mindistance = ed
		returntext += "%-8s %-8s %.3f %.4f ! From %-8s %-8s\n" % (missingbondtype[0], missingbondtype[1],params.bond_types[replacementtype].k,params.bond_types[replacementtype].req, replacementtype[0], replacementtype[1])
	...
	for missingangletype in missingset[2]:
		...
		if params.urey_bradley_types[replacementtype].k == 0:
			returntext += "%-8s %-8s %-8s %.3f %.4f ! From %-8s %-8s %-8s\n" % (missingangletype[0], missingangletype[1], missingangletype[2],params.angle_types[replacementtype].k,params.angle_types[replacementtype].theteq, replacementtype[0], replacementtype[1],replacementtype[2])
		else:
			returntext += "%-8s %-8s %-8s %.3f %.4f %.2f %.4f ! From %-8s %-8s %-8s\n" % (missingangletype[0], missingangletype[1], missingangletype[2],params.angle_types[replacementtype].k,params.angle_types[replacementtype].theteq, params.urey_bradley_types[replacementtype].k, params.urey_bradley_types[replacementtype].req, replacementtype[0], replacementtype[1],replacementtype[2])
	...
	for missingangletype in missingset[3]:
		...
		for prm in params.dihedral_types[replacementtype]:
			returntext += "%-8s %-8s %-8s %-8s %.4f %d %5.1f ! From %-8s %-8s %-8s %-8s\n" % (missingangletype[0], missingangletype[1], missingangletype[2], missingangletype[3],prm.phi_k, prm.per, prm.phase, replacementtype[0], replacementtype[1],replacementtype[2], replacementtype[3])
	return returntext+"\n"


prmlist = ["toppar/par_lignin.prm", "toppar/par_all36_cgenff.prm"]

fout = open("extraparameters.prm", "w")
fout.write(missingparameterstext(glob.glob("output/*psf"), prmlist))
fout.close()
